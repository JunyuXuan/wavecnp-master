...
from wavecnp2.utils import init_sequential_weights, BatchLinear, device
...
"""
    2D Conditional Neural Processes
"""


class ConditionalNeuralProcess2(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_channels,
                 use_attention=False):
        super(ConditionalNeuralProcess2, self).__init__()
        self.input_dim = input_dim
        self.latent_dim = latent_dim
    ...
    def forward(self, x, cnt_mask, tgt_mask_m, tgt_mask, cnt_num, tgt_num):
        ...
        Args:
            ...
            x_target (tensor): Target locations of shape
                `(batch, num_target, input_dim_x)`.
            size (int): Image size (image_size_1 x image_size_2)
        ...
        image_size_1 = x.shape[2]
        image_size_2 = x.shape[3]

        #
        x_cntxt = x * cnt_mask

        #
        x_context = torch.zeros(batch_size, cnt_num, 2).to(device)
        y_context = torch.zeros(batch_size, cnt_num, 1).to(device)
        x_target = torch.zeros(batch_size, tgt_num, 2).to(device)

        tgt_mask_matrix = tgt_mask.reshape(batch_size, image_size_1, image_size_2).int()

        for b in range(batch_size):
            x_cntxt_tmp = x_cntxt[b, 0, :, :]
            idx = cnt_mask[b, 0, :, :].nonzero(as_tuple=True)
            x_context[b, :, 0] = idx[0] * (2 / (image_size_1 - 1)) - 1
            x_context[b, :, 1] = idx[1] * (2 / (image_size_2 - 1)) - 1
            tmp = x_cntxt_tmp[idx]
            y_context[b, :, 0] = tmp

            idx = tgt_mask_matrix[b, :, :].nonzero(as_tuple=True)
            x_target[b, :, 0] = idx[0] * (2 / (image_size_1 - 1)) - 1
            x_target[b, :, 1] = idx[1] * (2 / (image_size_2 - 1)) - 1

        r = self.encode(x_context, y_context, x_target)

        mean, std = self.decode(x_target, r, 1)

        return mean.squeeze().flatten(), std.squeeze().flatten()
...
class RegressionCNP2(ConditionalNeuralProcess2):
    """Specialized version of the CNP for the image regression case.
    ...
    def __init__(self, latent_dim):
        super(RegressionCNP2, self).__init__(input_dim=2,
                                            latent_dim=latent_dim,
                                            num_channels=1)


class RegressionANP2(ConditionalNeuralProcess2):
    """Specialized version of the ANP for the image regression case.
    ...
    def __init__(self, latent_dim):
        super(RegressionANP2, self).__init__(input_dim=2,
                                            latent_dim=latent_dim,
                                            num_channels=1,
                                            use_attention=True)
