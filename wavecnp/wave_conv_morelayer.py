...
__all__ = ['ConvDeepSet', 'ConvCNP', 'WaveCNPnew']
...
class WaveCNPnew(nn.Module):
    ...
    def __init__(self,
        ...
        if self.TASK_DWT:
            # parameters of dwt
            self.level = Level          # int(log(num_points) / log(2.))
            self.wavename = 'db2'
            ...
            for i in range(self.level):
                self.conv_net_list.append(SimpleConv(self.h_dim, self.h_dim).to(device))
    ...
    def forward(self, x, y, x_out):
        ...
        if self.TASK_DWT:
            ...
            #
            for i in range(self.level):
                yh[i] = torch.zeros_like(yh[i])
                h = h + self.conv_net_list[i](self.iwt((yl, yh)))
