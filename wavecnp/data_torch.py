...
class DataGenerator(metaclass=abc.ABCMeta):
    ...
    def generate_task(self):
        ...
        # num_train_points = torch.randint(3, self.max_train_points + 1)
        # num_test_points = torch.randint(3, self.max_test_points + 1)
        # num_points = num_train_points + num_test_points
        ...
        for i in range(self.batch_size):
            ...
            task['y'].append(y[torch.argsort(x)])
            ...
            print(" ---- batch = ", i, " uses time: ", toc-tic)

        # Stack batch and convert to PyTorch.

        task = {k: torch.tensor(_uprank(torch.stack(v, dim=0)),
...
class GPGenerator(DataGenerator):
    ...
    def __init__(self, kernel=EQ(), **kw_args):
        self.gp = GP(kernel)
...
def preprocss_img_resolution_task(imgs, batch_size, image_size_1, image_size_2):
    ...
    for i in range(batch_size):
        ind = torch.randint(0, level+1, (1,)).item()
        tmp = imgs_new[i, ind, :, :]
