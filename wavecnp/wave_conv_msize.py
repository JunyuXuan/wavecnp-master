...
from wavecnp.architectures import UNet, SimpleMLP, SimpleConv, SimpleConv1, SimpleConv3
import math
import pywt
...
__all__ = ['ConvDeepSet', 'WaveDeepSet', 'ConvCNP', 'WaveCNP', 'WaveCNPnew']
...
class ConvCNP(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_net = SimpleConv3(self.h_dim, self.h_dim)
        self.num_points = num_points
...
class WaveCNP(nn.Module):
    ...
    def __init__(self,
        ...
        if self.TASK_DWT:
            ...
            for _ in range(self.level):
                ...
                self.filterband_transform_list.append(SimpleConv(self.h_dim, self.h_dim).to(device))
            ...
            #     if i == 0:
            #         self.filterband_transform_list.append(
            #             UNet().to(device))
            #     elif i == 1:
            #         self.filterband_transform_list.append(
            #             UNet().to(device))
            ...
            self.yl_transform = SimpleConv(self.h_dim, self.h_dim)
    ...
    def forward(self, x, y, x_out):
        ...
        if self.TASK_DWT:
            ...
            yl = self.yl_transform(yl)
            # total_params = sum(p.numel() for p in self.yl_transform.parameters())
            h = self.iwt((yl, yh))
        else:
            h = self.conv_net(h)
...
class WaveCNPnew(nn.Module):
    ...
    def __init__(self,
                 ...
                 TASK_DWT=False,
                 ADAPT=False):
        super(WaveCNPnew, self).__init__()
        self.activation = nn.Sigmoid()
        self.sigma_fn = nn.Softplus()
        self.h_dim = latent_dim
        self.conv_net = SimpleConv(self.h_dim, self.h_dim)
        ...
        if self.TASK_DWT:
            ...
            if ADAPT:
                self.wt_alpha = nn.Parameter(torch.tensor(math.pi / 3.0), requires_grad=True)
            else:
                self.wt_alpha = -1

            self.dwt = DWT1DForward(wave=self.wavename, J=self.level, adapt=ADAPT, L=self.L,
                                    wt_alpha=self.wt_alpha).to(device)
            self.iwt = DWT1DInverse(wave=self.wavename, adapt=ADAPT, L=self.L,
                                    wt_alpha=self.wt_alpha).to(device)

            self.conv_net2 = SimpleConv(self.h_dim, self.h_dim)
            self.conv_net1 = SimpleConv(self.h_dim, self.h_dim)
            self.conv_net0 = SimpleConv(self.h_dim, self.h_dim)
    ...
    def forward(self, x, y, x_out):
        ...
        if self.TASK_DWT:
            ...
            #
            h2 = self.conv_net2(h2)
            h1 = self.conv_net1(h1)
            h0 = self.conv_net0(h0)
