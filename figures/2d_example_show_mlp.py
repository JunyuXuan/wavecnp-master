...
load_dict = torch.load('results/pretrained_model_mlp/model_best.pth.tar', map_location=torch.device('cpu'), weights_only=False)
...
for x, (imgs, lbls) in enumerate(trainldr):
    ...
    half_img_size = int(image_size_1/2)
    ...
    cnt_mask_maxtrix = torch.zeros_like(imgs)
    cnt_mask_maxtrix[:, :, :, :half_img_size] = 1
    tgt_mask_maxtrix = torch.zeros_like(imgs)
    tgt_mask_maxtrix[:, :, :, half_img_size:] = 1

    cnt_num = half_img_size * img_size
    tgt_num = half_img_size * img_size

    tgt_mask_vec = torch.zeros_like(imgs)
    tgt_mask_vec[:, :, :, half_img_size:] = 1
    tgt_mask_vec = tgt_mask_vec.flatten()
    tgt_mask_vec = tgt_mask_vec > 0

    axs[1].imshow(np.transpose(torchvision.utils.make_grid(cnt_mask_maxtrix*imgs + tgt_mask_maxtrix, padding=0), (1, 2, 0)))
    ...
    # ### 0
    # y_mean = mean_list[0]
    # y_mean = (y_mean).reshape(batch_size, 1, 28, 14)
    # imgs_pred = torch.zeros_like(imgs)
    # imgs_pred[:, :, :, 14:] = y_mean
    # imgs_0 = imgs * cnt_mask_maxtrix + imgs_pred
    #
    # axs[2].imshow(np.transpose(torchvision.utils.make_grid(imgs_0, padding=0), (1, 2, 0)))
    # # axs[2].text(-25, 15, 'WaveCNP', horizontalalignment='center')
    # axs[2].text(0, -2, 'Level 1', fontsize=15)

    ### 1
    y_mean = mean_list[1]
    y_mean = (y_mean).reshape(batch_size, 1, img_size, half_img_size)
    imgs_pred = torch.zeros_like(imgs)
    imgs_pred[:, :, :, half_img_size:] = y_mean
    imgs_1 = imgs * cnt_mask_maxtrix + imgs_pred

    axs[2].imshow(np.transpose(torchvision.utils.make_grid(imgs_1, padding=0), (1, 2, 0)))
    # axs[2].text(-25, 15, 'WaveCNP', horizontalalignment='center')
    axs[2].text(0, -2, 'Level 1', fontsize=15)
    ...
    imgs_pred = torch.zeros_like(imgs)
    imgs_pred[:, :, :, half_img_size:] = y_mean
    imgs_2 = imgs * cnt_mask_maxtrix + imgs_pred

    axs[3].imshow(np.transpose(torchvision.utils.make_grid(imgs_2, padding=0), (1, 2, 0)))
    # axs[2].text(-25, 15, 'WaveCNP', horizontalalignment='center')
    ...
    imgs_pred = torch.zeros_like(imgs)
    imgs_pred[:, :, :, half_img_size:] = y_mean
    imgs_3 = imgs * cnt_mask_maxtrix + imgs_pred

    axs[4].imshow(np.transpose(torchvision.utils.make_grid(imgs_3, padding=0), (1, 2, 0)))
    # axs[2].text(-25, 15, 'WaveCNP', horizontalalignment='center')
...
# fig.savefig('_experiments/figs/2d_mlp.pdf',bbox_inches='tight')
