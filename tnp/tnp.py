...
from tnp.modules import build_mlp
...
class TNP(nn.Module):
    ...
    def construct_input(self, batch, autoreg=False):
        x_y_ctx = torch.cat((batch.x_context, batch.y_context), dim=-1)
        x_0_tar = torch.cat((batch.x_target, torch.zeros_like(batch.y_target)), dim=-1)
        if not autoreg:
            inp = torch.cat((x_y_ctx, x_0_tar), dim=1)
        else:
            if self.training and self.bound_std:
                yt_noise = batch.y_target + 0.05 * torch.randn_like(batch.y_target) # add noise to the past to smooth the model
                x_y_tar = torch.cat((batch.x_target, yt_noise), dim=-1)
            else:
                x_y_tar = torch.cat((batch.x_target, batch.y_target), dim=-1)
            inp = torch.cat((x_y_ctx, x_y_tar, x_0_tar), dim=1)
        return inp

    def create_mask(self, batch, autoreg=False):
        num_ctx = batch.x_context.shape[1]
        num_tar = batch.x_target.shape[1]
